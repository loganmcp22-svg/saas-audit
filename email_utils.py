...
def send_password_reset_email(user_email, token):
    """Send a password-reset link. Returns (ok: bool, error: str|None)."""
    api_key = os.environ.get('SENDGRID_API_KEY', '')
    if not api_key:
        return False, 'SENDGRID_API_KEY is not set'

    reset_url = f'https://subaudit.io/reset-password/{token}'
...
<body style="margin:0;padding:0;background:#f8fafc;font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',Roboto,sans-serif;">
  <table width="100%" cellpadding="0" cellspacing="0" style="background:#f8fafc;padding:40px 20px;">
      ...
      <table width="100%" style="max-width:560px;background:#ffffff;border-radius:16px;border:1px solid #e2e8f0;box-shadow:0 1px 4px rgba(0,0,0,0.05);">
        ...
        <tr>
          <td style="padding:32px;">
            ...
            <p style="font-size:14px;color:#64748b;margin:0 0 28px;">
              We received a request to reset the password for your SubAudit account.
    ...
    text = (
        'We received a request to reset the password for your SubAudit account.\n\n'
        'Choose a new password by visiting this link (expires in 1 hour):\n'
        f'{reset_url}\n\n'
        "If you didn't request a password reset, you can safely ignore this email — "
        'your password will not be changed.'
    )
    ...
    message = Mail(
        from_email=FROM_EMAIL,
        to_emails=user_email,
        subject='Reset your password — SubAudit',
        html_content=html,
        plain_text_content=text,
    )
